...
class ProdigyPlusScheduleFree(torch.optim.Optimizer):
    ...
    def step_param(self, p, group):
        ...
        if p.grad is not None:
            ...
            self.running_d_numerator.add_(torch.dot(sliced_grad, x0_minus).abs_(), alpha=d_update)
            del x0_minus
    ...
    def step(self, closure=None):
        ...
        for group in groups:
            ...
            for p in active_p:
                ...
                x0_minus = state['p0'] - sliced_data
                d_numerator_accum.add_(torch.dot(sliced_grad, x0_minus).abs_(), alpha=d_update)
                del x0_minus
